...
import { kling4kI2vRouteInputSchema } from "@/lib/freepik/kling-4k-schema";
...
/**
 * POST /api/freepik/kling-4k-i2v
 * Header: Authorization: Bearer <activation-code>
 * Body:   { params: Kling4kI2vGenerateParams }
 *
 * Same orchestrator flow as kling-4k-t2v but with the image-driven
 * request shape (image required, optional image_tail / motion masks).
 */
export async function POST(request: Request) {
  const body = await parseJsonBody(request);
  const parsed = kling4kI2vRouteInputSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const lookup = lookupForKling4k("kling-4k-i2v", params);
  let cost: number;
  try {
  ...
  const result = await orchestrateFreepikCall({
    ...
    withAudio: false,
    callFreepik: (apiKey) => freepik.kling4k.generateI2v(params, { apiKey }),
